...
describe('ActivtyTableRow', function() {
  let TestRow = ReactTestUtils.renderIntoDocument(
    <ActivityTable
      loading={false}
  ...
  it('renders a table row with activity-table-row class and closed class', function() {
    let rows = ReactTestUtils.scryRenderedDOMComponentsWithTag(TestRow, 'tr');
    // rows[0] is header row
    return expect(rows[1].className).to.eq('activity-table-row closed');
  });

  return it('changes class open to class closed when state is_open', function() {
    let row = ReactTestUtils.scryRenderedDOMComponentsWithClass(TestRow, 'activity-table-row')[0];
    expect(row.className).to.eq('activity-table-row closed');
    return click(row).then(row => expect(row.className).to.eq('activity-table-row open'));
  });
});

...
# text units (usually the text added by a revision or diff) that
# ScoreAiDetectionSample later sends to one or more AI detectors. Subclasses
# enumerate candidate units and call #add_url_unit, #add_revision_unit or
# #add_text_unit; this class fetches the plaintext once, skips units without
# enough prose, and never adds the same unit to a sample twice. Nothing here
# calls a detector.
class BuildAiDetectionSample
  ...
  def self.ground_truth_for_term(slug)
    ...
    AiDetectionSample::HUMAN_PRE_LLM if pre_llm
  end
  ...
  # For text that did not come from a revision, such as experiment output.
  ...
  def create_unit(**attrs)
    ...
    log "added unit #{unit.id} (#{unit.word_count} words): #{unit.url}"

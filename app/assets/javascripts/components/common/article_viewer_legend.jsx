...
const ArticleViewerLegend = ({ article, users, colors, status, allUsers, failureMessage, unhighlightedEditors }) => {
    ...
    if (users) {
      ...
      setUserLinks(users.map((user, i) => {
        ...
        if (status === 'loading') {
          ...
          res = <div key={`legend-${user.name}`} className={`user-legend-article-viewer user-legend-name ${colors[i]}`}><a href={userLink} title={realName} target="_blank">{user.name}</a><img className="user-legend-hover" style={{ color: 'transparent' }} src="/assets/images/arrow.svg" alt="scroll to users revisions" width="30px" height="20px" onClick={() => Scroller.scrollTo(user.name, scrollBox)} /></div >;
        } else if (UnhighlightedContributions) {
          res = <div key={`legend-${user.name}`} className={'user-legend-article-viewer tooltip-trigger'}><p className={'tooltip large'} id={'popup-style'} >{I18n.t('users.contributions_not_highlighted', { username: user.name })}</p><a href={userLink} title={realName} target="_blank">{user.name}</a>{<span className="tooltip-indicator-article-viewer" />}</div>;
        } else {
          res = <div key={`legend-${user.name}`} className={'user-legend-article-viewer tooltip-trigger'}><p className={'tooltip large'} id={'popup-style'}>{I18n.t('users.no_highlighting', { editor: user.name })}</p><a href={userLink} title={realName} target="_blank">{user.name}</a>{<span className="tooltip-indicator-article-viewer" />}</div>;
        }

        return res;
      }));
    } else {
      setUserLinks(<div className="user-legend-article-viewer authorship-loading"> &nbsp; &nbsp; </div>);
    }
  ...
  useEffect(() => {
    if (status === 'loading') {
      setUsersStatus(
        <div>
          <div className="user-legend-article-viewer authorship-loading"> &nbsp; &nbsp; </div>
          <div className="user-legend-article-viewer authorship-status">{I18n.t('users.loading_authorship_data')}</div>
          <div className="user-legend-article-viewer authorship-loading"> &nbsp; &nbsp; </div>
        </div>
      );
    } else if (status === 'failed') {
      setUsersStatus(<div className="user-legend-article-viewer authorship-status-failed">{I18n.t('users.authorship_data_not_fetched')}: {failureMessage}</div>);
  ...
  return (
    <div className="user-legend-wrap">
      <div className="user-legend-article-viewer">{I18n.t('users.edits_by')}&nbsp;</div>
      {userLinks}
      {usersStatus}
    </div>
  );

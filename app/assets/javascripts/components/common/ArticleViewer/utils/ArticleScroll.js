...
export class ArticleScroll {
  ...
  createScrollObject(titles, users) {
    ...
    // Adds paragraph containing highlighting along with position and index
    // values to scrollObject
  ...
  scrollTo(name, scrollBox) {
    ...
    // Check Minimum distance, this prevents the scroll from only scrolling
    // short distances
    ...
    const targetParagraph = paragraphs[index].paragraph;
    const id = this.findId(targetParagraph);
    ...
    if (this.bump === true || length === 1) {
      ...
      // Scrolling function, either scrolls edit to the top of the page or
      // scrolls edit till in view at bottom of page
    ...
    // NEW: return the element we scrolled to so callers can move focus
  ...
  findInView(name, paragraphs, length, scrollBox) {
    ...
    while (this.currentCoordinate !== null && this.isInView(
               scrollBox, this.currentCoordinate, this.nextPosition) === true) {
      this.scrollObject[name].index += 1;
  ...
  findDistance(name, paragraphs, length) {
    while (this.checkDistanceMin(this.currentCoordinate, this.nextPosition) === false) {
      ...
      if (this.checkDistanceMax(
              this.nextPosition,
              paragraphs[this.scrollObject[name].index].coordinates) === true) {
        this.scrollObject[name].index -= 1;
        this.nextPosition = paragraphs[this.scrollObject[name].index].coordinates;
        break;
      }
  ...
  isInView(window, curr, next) {
    if (curr.top + window.clientHeight - 50 > next.bottom && curr.bottom < next.top) {
      return true;
    }
    return false;
  }
  ...
  // Quickly bumps the scroll window down to show the are now relevant edits
  // below
  scrollBump(id) {

...
const ArticleFinderRow = (props) => {
  ...
  if (props.courseSlug && props.current_user.isAdvancedRole) {
    if (props.assignment) {
      ...
      button = (
        <td>
          <button type="button" className={className} onClick={() => unassignArticle()}>{I18n.t(`article_finder.${ArticleUtils.projectSuffix(props.selectedWiki.project, 'remove_article')}`)}</button>
        </td>
      );
    } else {

...
OrderableBlock = React.createClass(
  ...
  render: ->
    <div className="block block--orderable" style={{opacity: if @props.isDragging then .5 else 1}}>
      ...
      <p>{@props.kind}</p>
      <button onClick={@props.onMoveDown} className="button border" disabled={@props.disableDown}><i className="icon icon-arrow-down"/></button>
      <button onClick={@props.onMoveUp} className="button border" disabled={@props.disableUp}><i className="icon icon-arrow-up"/></button>
    </div>

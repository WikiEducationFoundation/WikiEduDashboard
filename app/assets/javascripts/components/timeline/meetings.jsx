...
const Meetings = ({ weeks, course }) => {
  Meetings.propTypes = {
    ...
    onPersistCourse: PropTypes.func.isRequired,
  ...
  const saveDisabledClass = (course) => {
    ...
    const enable = blackoutDatesSelected || (anyDatesSelected && course.no_day_exceptions);
  ...
  useEffect(() => {
    const shouldDisableSave = !valid || !course.start || !course.end;
    setSaveDisabled(shouldDisableSave);
  }, [valid, course.start, course.end]);

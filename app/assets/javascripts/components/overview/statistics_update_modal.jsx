...
const StatisticsUpdateModal = (props) => {
  ...
  if (course.updates.last_update) {
    errorCount = course.updates.last_update.error_count;
  }
  if (errorCount === null) {
    lastUpdateSummary = 'SOME MSG';
  ...
  let recentUpdatesSummary = '';
  let updateNumbers = '';
  let totalUpdatesMessage = '';
  ...
  recentUpdatesSummary = I18n.t('metrics.recent_updates_summary', { total: updateLogs.length, failure_count: failureUpdatesCount, error_count: erroredUpdatesCount });
  ...
  updateNumbers = Object.keys(course.flags.update_logs);

  totalUpdatesMessage = `${I18n.t('metrics.total_updates')}: ${updateNumbers[updateNumbers.length - 1]}`;
}

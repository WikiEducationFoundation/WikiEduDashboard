...
const ActivityTableRow = createReactClass({
  ...
  render() {
    ...
    if (this.props.diffUrl) {
      revisionDateTime = (
        <a target="_blank" href={this.props.diffUrl}>{this.props.revisionDateTime}</a>
      );
    }
    ...
    return (
      <tr className={className} key={this.props.rowId}>
        <td onClick={this.openDrawer}>
          <a target="_blank" href={this.props.articleUrl}>{this.props.title}</a>
        </td>
        {col2}
        <td onClick={this.openDrawer}>
          <a target="_blank" href={this.props.talkPageLink}>{this.props.author}</a>
        </td>
        <td onClick={this.openDrawer}>
          {revisionDateTime}
        </td>

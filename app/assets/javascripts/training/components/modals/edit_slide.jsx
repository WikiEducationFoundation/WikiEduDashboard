...
import { updateSlide } from '../../../actions/training_modification_actions.js';
...
const EditSlide = (props) => {
  ...
  useEffect(() => {
    setSlide({
      slug: props.slide.slug,
      title: props.slide.title,
      wiki_page: props.slide.wiki_page,
    });
  ...
  const submitHandler = () => {
    ...
    if (validateFields()) {
      setSubmitting(true);
      dispatch(updateSlide(props.library_id, props.module_id, slide, setSubmitting));
    }
  };
      ...
      <Modal >
        <div className="container training-modification">
          <div className="wizard__panel active training_modal single_column small-container" style={formStyle}>
            <h3>{I18n.t('training.update_slide')}</h3>
            <p>{I18n.t('training.update_slide_msg')}</p>
            <div>
              <TextInput
                id="slug"
              ...
              <button className="button dark right" onClick={submitHandler}>{I18n.t('training.update')}</button>
            </div>
          </div>
        </div>
      </Modal>

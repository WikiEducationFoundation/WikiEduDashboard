...
module ApplicationHelper
  ...
  def css_fingerprinted(filename)
    manifest_path = Rails.public_path.join('assets/javascripts/manifest.json')
    manifest = Oj.load(File.read(File.expand_path(manifest_path, __FILE__)))
  ...
  def en_if_invalid(locale)
    ...
    return 'en' unless Rails.public_path.join("assets/javascripts/i18n/#{locale}.js").exist?
    locale
  end

  def i18n_javascript_tag(locale)
    locale = en_if_invalid(locale)
    md5 = Digest::MD5.file(Rails.public_path.join("assets/javascripts/i18n/#{locale}.js")).hexdigest

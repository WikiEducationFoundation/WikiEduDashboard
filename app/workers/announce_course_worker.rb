...
class AnnounceCourseWorker
  ...
  def self.schedule_announcement(course:, editing_user:, instructor:, action: 'add_course_template_to_instructor_userpage')
    perform_async(course.id, editing_user.id, instructor.id, action)
  end

  def perform(course_id, editing_user_id, instructor_id, action = 'add_course_template_to_instructor_userpage')

...
describe UpdateLogger do
  describe '.update_settings_record' do
    it 'adds the time of the metrics last update as a value to the settings table' do
      UpdateLogger.update_settings_record('start_time' => Time.now, 'end_time' => Time.now)
    ...
    it 'adds the average delay time to the settings table' do
      UpdateLogger.update_settings_record('start_time' => 14.hours.ago, 'end_time' => 12.hours.ago)
      UpdateLogger.update_settings_record('start_time' => 12.hours.ago, 'end_time' => 9.hours.ago)
      UpdateLogger.update_settings_record('start_time' => 9.hours.ago, 'end_time' => 8.hours.ago)
      UpdateLogger.update_settings_record('start_time' => 8.hours.ago, 'end_time' => 6.hours.ago)
      record = Setting.find_or_create_by(key: 'metrics_update')
    ...
    it 'does not set average delay if there was only one update' do
      UpdateLogger.update_settings_record('start_time' => 10.hours.ago, 'end_time' => 8.hours.ago)
      record = Setting.find_or_create_by(key: 'metrics_update')
      delay = record.value['average_delay']

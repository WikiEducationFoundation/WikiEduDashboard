...
describe StudentGreetingChecker do
  ...
  it 'pulls the usernames of all greeters' do
    expect(described_class.new.instance_variable_get(:@usernames)).to eq(['Danny'])
  end
  ...
  describe '.check_all_ungreeted_students' do
    ...
    describe '.check' do
      ...
      let(:checker) { StudentGreetingChecker::Check.new(User.find(2), wiki, greeters, usernames) }

...
require Rails.root.join('lib/analytics/course_uploads_csv_builder')
...
describe CourseUploadsCsvBuilder do
  ...
  before do
    ...
    upload_count.times do |i|
      create(:commons_upload, file_name: "File:#{i}.gif", user:,
                              uploaded_at: course.start + 1.minute)
    end
    # Upload outside the course
    create(:commons_upload, file_name: 'File:Nope.gif', user:,
                            uploaded_at: course.start - 1.minute)
  end

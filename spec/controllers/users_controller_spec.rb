...
describe UsersController do
  ...
  describe '#enroll' do
    ...
    let(:request_params) {{ course_id: course.slug, passcode: course.passcode, titleterm: 'foobar' }}
    ...
    context 'POST' do
      let(:post_params) {{ id: course.slug, user: { user_id: user.id, role: 0 }.as_json }}
    ...
    context 'DELETE' do
      let(:delete_params) {{ id: course.slug, user: { user_id: user.id, role: 0 }.as_json }}
      before do
        CoursesUsers.create(user_id: user.id, course_id: course.id, role: 0)
        article = create(:article)
        create(:assignment, course_id: course.id, user_id: user.id, article_id: article.id)

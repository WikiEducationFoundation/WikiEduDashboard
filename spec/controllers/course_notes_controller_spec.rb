...
RSpec.describe CourseNotesController, type: :controller do
  describe 'GET #show' do
    it 'returns a success response with course notes' do
      ...
      course_note = create(:course_note, courses_id: course.id)
  ...
  describe 'POST #create' do
    ...
    it 'returns unprocessable entity on creation failure' do
      allow_any_instance_of(CourseNote).to receive(:create_new_note).and_return(false)

      post :create, params: { course_note: { courses_id: 1, title: 'New Note',
      text: 'Note Text', edited_by: 'User' } }

      expect(response).to have_http_status(:unprocessable_entity)

      json_response = JSON.parse(response.body)
      expect(json_response['error']).to eq('Failed to create course note')
    end
  end

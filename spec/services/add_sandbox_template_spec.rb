...
describe AddSandboxTemplate do
  ...
  let(:no_default_sandbox_template) { 'No default or sandbox template present.' }
  ...
  it 'does not add sandbox template twice' do
    ...
    described_class.new(home_wiki: course.home_wiki, sandbox: sandbox,
                        sandbox_template: @sandbox_template, current_user: user)
  end
  ...
  it 'adds sandbox template' do
    allow_any_instance_of(WikiApi).to receive(:get_page_content).and_return(no_default_sandbox_template)

...
if ! which curl >/dev/null; then
  ...
  output_line "sudo dnf install -y curl" && printf "${CLEAR_LINE}[+] Curl installed \n"
else
...
if which R > /dev/null; then
  ...
  output_line "sudo dnf install -y R-core" && printf "${CLEAR_LINE}[+] R installed\n"
fi
...
if which node >/dev/null;then
  ...
  output_line "sudo dnf install -y nodejs" && printf "${CLEAR_LINE}[+] Node installed\n"
fi

printf '[*] Installing GNUpg... \n'
output_line "sudo dnf install -y gnupg" && printf "${CLEAR_LINE}[+] GNUpg installed\n"
...
if which yarn >/dev/null; then
  ...
  output_line "sudo dnf install -y yarn" && printf "${CLEAR_LINE}[+] Yarn installed\n"
fi
...
if which pandoc >/dev/null; then
  ...
  output_line "sudo dnf install -y pandoc" && printf "${CLEAR_LINE}[+] Pandoc installed\n"
fi
...
if which redis-server > /dev/null; then
  ...
  output_line "sudo dnf install -y redis" && printf "${CLEAR_LINE}[+] Redis-Server installed\n"
fi
...
if mysql -V | grep MariaDB > /dev/null; then
  ...
  output_line "sudo dnf install -y mariadb-server" && printf "${CLEAR_LINE}[+] MariaDB installed "
fi

printf '[*] Installing mariadbclient dependencies... \n'
output_line "sudo dnf install -y mariadb-devel" && printf "${CLEAR_LINE}[+] Dependencies installed\n"

...
class UploadImporter
  ...
  def self.update_usage_count(commons_uploads, update_cs: nil)
    commons_uploads.in_groups_of(50, false) do |file_batch|
      usages = Commons.get_usages(file_batch, update_cs: update_cs)
  ...
  # Data methods #
  ################
  def self.import_urls_in_batches(commons_uploads, update_cs: nil)
    # Larger values (50) per batch choke the MediaWiki API on this query.
    commons_uploads.in_groups_of(10, false) do |file_batch|
      file_urls = Commons.get_urls(file_batch, update_cs: update_cs)

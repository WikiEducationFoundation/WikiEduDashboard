...
| **2.4.1 Bypass Blocks** | Supports | Pages use HTML5 landmarks for assistive-tech navigation; the application layout sets `%main#main{:role => "main"}` with `<nav>` and `<header>` in the shared partials. Landmark-based skip is sufficient to satisfy this criterion per WCAG technique ARIA11. **Note:** there is no visible "skip to main content" link in any layout; this is a usability gap for sighted keyboard users (and a common addition for VPAT-grade attestations) but is not a 2.4.1 failure. |
...
| **3.3.1 Error Identification** | Partially Supports | Server-side form errors are rendered as text in the page (often via Rails' `errors.full_messages` in HAML partials), so they are discoverable in document order. **Verified gap:** no view template or React form component in the codebase uses `aria-invalid` or `aria-required` on the failing field, and `aria-describedby` is used in only three places (none of them for form errors). Errors are identifiable by reading the page but are not programmatically associated to their fields. |
...
| **4.1.2 Name, Role, Value** | Partially Supports | axe-locked pages enforce ARIA attribute validity and accessible-name presence on interactive controls. `jsx-a11y/aria-role` and `jsx-a11y/aria-props` enforced in the React layer. **Verified gaps:** the shared `Modal` component (`app/assets/javascripts/components/common/modal.jsx`) renders a `<div>` without `role="dialog"` or `aria-modal`; the `Confirm` variant uses `role="alert"` which is semantically incorrect for a dialog requiring user response. Custom widgets (drag-and-drop, multi-step wizard step indicators) have not been comprehensively audited. |
...
| **1.3.5 Identify Input Purpose** | Does Not Support | **Verified gap:** zero occurrences of the HTML `autocomplete` attribute on form inputs anywhere in the view templates or React components (the only "autocomplete" matches in the codebase are CSS class names for search dropdowns, not the WCAG-relevant attribute). Login, account-settings, and other forms collecting user information do not declare input purpose programmatically. This is a small, scoped remediation. |
...
| **2.4.7 Focus Visible** | Partially Supports | The site-wide CSS reset includes `button:focus { outline: none }` (`app/assets/stylesheets/_reset.styl:75`), which suppresses the default focus ring on all `<button>` elements. Many specific button modules and form controls define a replacement `:focus` style (training, surveys, FAQ, home, article-finder, etc.), so most buttons do have a visible focus indicator — but the coverage is per-component rather than universal, and the reset means any newly-added button without explicit focus styling will fail this criterion. Admin and core course-page surfaces validated through daily JAWS use (which does not directly validate the visible focus indicator but does confirm the focus order itself is sound). |
